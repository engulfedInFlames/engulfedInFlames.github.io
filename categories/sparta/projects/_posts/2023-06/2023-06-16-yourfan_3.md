...
- `-v` 태그에 [로컬상의 프로젝트 절대 경로:컨테이너상 경로]를 매칭하여 싱크할 수 있음
- 로컬 프로젝트에 변화가 생기면 컨테이너상의 프로젝트에도 즉각 반영
- 역방향으로도 작용
- 역방향으로 '쓰기' 권한을 제거하려면 `:ro`를 추가하여 *read-only*로 싱크시킬 수 있음
...
- 기존의 도커 파일과 구분하여 *Dockerfile.prod*를 생성
- 멀티 스테이지 빌드를 위해 도커 파일 재구성
...
- 로컬호스트의 8080번 포트로 접속하여 정상 연결되는지 확인
...
&ensp; 개발 도중이지만, 멀티 스테이지 빌드에 기반하여 도커 컴포즈로 실행까지 완료했습니다. 이제 다시 개발 환경에서 백엔드와의 통신을 완료한 뒤에, 백엔드도 도커 컴포즈에 포함시키기만 하면 될 것 같습니다.
