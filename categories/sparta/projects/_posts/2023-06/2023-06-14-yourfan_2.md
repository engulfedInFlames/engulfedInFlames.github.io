...
- 액션에 리다이렉트 액션을 추가하고 'HTTPS | 443'으로 지정
...
&ensp; 저는 두 번째 트러블슈팅을 하고 서야 제가 빌드한 앱이 제대로 렌더링되기 시작했습니다. 그전까지는 제 앱이 아닌, NGINX의 기본 *index.html*이 렌더링되어 골머리를 앓게 했었습니다만, 이 _default_ 파일을 삭제하고 나니 제 앱이 렌더링되더군요.
