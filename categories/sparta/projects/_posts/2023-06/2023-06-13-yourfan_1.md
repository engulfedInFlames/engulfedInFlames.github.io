...
- `eb init --platform node.js --region ap-northeast-2` 실행 결과 &rarr;
...
- `eb create --sample yourfan-frontend` 실행 결과 &rarr;
...
&ensp; 그러나 502 오류는 해결되지 않았습니다. `eb logs` 명령어를 실행해서 로그를 확인하니 다음을 확인할 수 있었습니다.
